...
```
VibedActor/
  bazos-scraper/          # Apify actor — scrapes bazos.cz classifieds
    src/
      main.py             # Scraper entry point
      database.py         # DB operations (SHARED — keep in sync with gfr-scraper)
  gfr-scraper/            # Apify actor — scrapes Czech gov auctions (drazby.fs.gov.cz)
    src/
      main.py             # Scraper entry point
      database.py         # DB operations (SHARED — keep in sync with bazos-scraper)
      czech_cities.py     # Static Czech city -> lat/lng lookup for geocoding
  listings-visualization/ # Express.js web app (map, admin, scheduler)
    ...
    public/
      ...
      common.js           # Shared auth check, hamburger menu, message utils
      style.css           # All styles (no inline CSS in HTML files)
  dbcheck/
    migration.sql         # Current migration (run against live DB)
  docker-compose.yml      # Docker Compose for Elestio CI/CD deployment
...
When editing one, copy changes to the other.
...
The visualization app runs on Elestio via Docker with auto-deploy on every GitHub push.

**Files:**
- `docker-compose.yml` — builds `listings-visualization/Dockerfile`, exposes port 3000
- `elestio.yml` — Elestio config: `dockerCompose` runtime, HTTPS reverse proxy (443 → 3000)
- `listings-visualization/Dockerfile` — Node 20 Alpine, `npm ci --omit=dev`, runs `node server.js`

**Setup steps:**
1. Push to GitHub (main branch)
2. Elestio dashboard → **CI/CD** → **GitHub** → select this repo
3. Elestio auto-detects `elestio.yml` + `docker-compose.yml`
4. Set secrets in Elestio dashboard env vars: `DB_PASSWORD`, `APIFY_TOKEN`
5. Deploy — app is live at `https://[elestio-domain]/` with automatic SSL

**Auto-deploy:** Every push to the connected branch triggers a rebuild and redeploy (Elestio sets up a GitHub webhook automatically).

**Environment variables:** Defaults are pre-filled in `elestio.yml`. `SESSION_SECRET` auto-generates via Elestio's `random_password` token. `DB_PASSWORD` and `APIFY_TOKEN` must be set manually in the dashboard.
